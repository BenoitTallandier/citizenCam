...
while(cap.isOpened()):
    ...
    for c in contours:
        ...
        if h>20 and w>20 :
            somme = 0
            compteur = 0;
            bleu = int(np.mean(frame[y:y+h,x:x+h,0:1]))
            vert = int(np.mean(frame[y:y+h,x:x+h,1:2]))
            rouge = int(np.mean(frame[y:y+h,x:x+h,2:3]))

            moyenne = "(%d,%d,%d)" %(rouge,vert,bleu)
            if rouge > bleu+10 and rouge > vert+10:
                message = "rouge"
                cv2.putText(frameAffiche,message,(x+w+10,y+h),0,0.3,(0,0,255))
            elif bleu>rouge+10 and bleu>vert+10:
                message = "bleur"
